...
$Header: //source/qcom/qct/core/pkg/mpss/rel/1.1.4/modem_proc/core/api/services/list.h#1 $ $DateTime: 2012/06/12 17:24:45 $ $Author: coresvc $
...
typedef struct {
   ...
   #ifdef FEATURE_QDSP6
      blast_mutex_t pkcsListCriticalSection;
   #else
      #ifdef FEATURE_WINCE
#error code not present
      #else
         #ifdef USE_UCGUI_SIMULATOR
         //CRITICAL_SECTION pkcsListCriticalSection;
         #else
         pthread_mutex_t pkcsListCriticalSection;
         #endif
      #endif   
   #endif
} list_type;
...
*/
#ifdef FEATURE_QDSP6
   #define LIST_MUTEX_LOCK(pkcs) blast_pimutex_lock(&pkcs)
#else /* FEATURE_QDSP6 */
   #ifdef FEATURE_WINCE
#error code not present
   #else
      #ifdef USE_UCGUI_SIMULATOR
      #define LIST_MUTEX_LOCK(pkcs) //EnterCriticalSection(&(pkcs))
      #else
      #define LIST_MUTEX_LOCK(pkcs) pthread_mutex_lock(&(pkcs))
      #endif
   #endif /* FEATURE_WINCE */
#endif /* FEATURE_QDSP6 */
/** @endcond */
...
*/
#ifdef FEATURE_QDSP6
   #define LIST_MUTEX_FREE(pkcs) blast_pimutex_unlock(&pkcs)
#else /* FEATURE_QDSP6 */
   #ifdef FEATURE_WINCE
#error code not present
   #else /* FEATURE_WINCE */
      #ifdef USE_UCGUI_SIMULATOR
      #define LIST_MUTEX_FREE(pkcs) //LeaveCriticalSection(&(pkcs))
      #else
      #define LIST_MUTEX_FREE(pkcs) pthread_mutex_unlock(&(pkcs))
      #endif
#endif /* FEATURE_WINCE */
#endif /* FEATURE_QDSP6 */
/** @endcond */
