...
$Header: //source/qcom/qct/core/pkg/mpss/rel/1.1.4/modem_proc/core/services/utils/src/list.c#1 $ $DateTime: 2012/06/12 17:24:45 $ $Author: coresvc $

when       who     what, where, why
--------   ---     ---------------------------------------------------------
05/14/12    rks    Move from legacy qmutex_lock/unlock to blast_pimutex_lock/unlock
...
#error code not present
#else 
  #ifndef USE_UCGUI_SIMULATOR
  //#include "rex.h"
  #endif
#endif
//#ifdef USE_UCGUI_SIMULATOR
...
// Mutex Initialisation
    ...
    blast_pimutex_init(&(list_ptr->pkcsListCriticalSection));
...
#error code not present
   #else /* FEATURE_WINCE */
      #ifdef USE_UCGUI_SIMULATOR
      //InitializeCriticalSection(&(list_ptr->pkcsListCriticalSection));
      #else
      pthread_mutex_init(&(list_ptr->pkcsListCriticalSection), NULL);
      #endif
   #endif /* FEATURE_WINCE */
#endif /* FEATURE_QDSP6 */
...
)
{
   ...
   LIST_ASSERT( list_ptr->size == 0);

/* Mutex Initialisation */
#ifdef FEATURE_QDSP6

   blast_pimutex_destroy(&(list_ptr->pkcsListCriticalSection));
...
#error code not present
   #else /* FEATURE_WINCE */
      #ifdef USE_UCGUI_SIMULATOR
      //DeleteCriticalSection( &(list_ptr->pkcsListCriticalSection) );
      #else
      pthread_mutex_destroy( &(list_ptr->pkcsListCriticalSection) );
      #endif
   #endif /* FEATURE_WINCE */
#endif /* FEATURE_QDSP6 */
   list_ptr->back_ptr         = NULL;
