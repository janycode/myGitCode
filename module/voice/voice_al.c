...
#include "jrd_qmi_common.h"
#include "jrd_qmi_voice.h"
#include "jrd_voice_al.h"

static list_type jrd_voice_event_list = {0};
static jrd_voice_call_info_t g_call_info = {0};
static jrd_voice_call_info_t g_third_call_info = {0};

static pthread_t jrd_dtmf_thrd;

#define JRD_VOICE_DEBUG

/*===========================================================================
  Function:  jrd_voice_al_decode_call_info
===========================================================================*/
/*!
@brief
  None.
...
*/
/*=========================================================================*/
static int jrd_voice_al_decode_call_info(voice_call_info2_type_v02 *src_data, jrd_voice_call_info_t *decode_data)
{
    e_jrd_voice_call_state_t call_state = E_JRD_VOICE_CALL_STATE_INVALID;
    e_jrd_voice_call_type_t call_type = E_JRD_VOICE_CALL_TYPE_INVALID;
    e_jrd_voice_call_direction_t call_direction = E_JRD_VOICE_CALL_DIRECTION_INVALID;
    e_jrd_voice_call_mode_t call_mode = E_JRD_VOICE_CALL_MODE_INVALID;
    int call_id = 0;
...
*/
/*=========================================================================*/
static void jrd_voice_al_msg_cb(
  unsigned int msg_id,
  void *msg_data,
  unsigned int msg_data_len)
{
    list_link_type *list_node_link = NULL;
    jrd_voice_event_list_node_t *this_msg_node = NULL;
    e_jrd_voice_event_t event = E_JRD_VOICE_EVENT_INVALID;
    ...
    switch (msg_id)
    {
        case QMI_VOICE_DTMF_IND_V02:
        {
            ...
            {
                pthread_cond_signal (&jrd_net_signal.cond);
                #ifdef JRD_VOICE_DEBUG
        ...
        case QMI_VOICE_ALL_CALL_STATUS_IND_V02:
        {
            ...
            jrd_voice_call_info_t call_info[QMI_VOICE_CALL_INFO_MAX_V02] = {0};
            int index = 0;

            JRD_MALLOC(sizeof(jrd_voice_call_info_t), event_data);
            if (!event_data) {
            ...
            for (index = 0; index < QMI_VOICE_CALL_INFO_MAX_V02; index++) {
                rc = jrd_voice_al_decode_call_info(&(voice_call_status_msg_data->call_info[index]), &call_info[index]);
                if (rc != JRD_NO_ERR) {
                ...
                event_data_len = sizeof(jrd_voice_call_info_t);
            ...
            if (index == 2) {
                ...
                JRD_MEMCPY(&g_third_call_info, &call_info[0], sizeof(jrd_voice_call_info_t));
                JRD_MEMCPY(&g_call_info, &call_info[1], sizeof(jrd_voice_call_info_t));
                ...
                JRD_MEMCPY(event_data, &call_info[0], event_data_len);
                JRD_MEMCPY(&g_call_info, &call_info[0], sizeof(jrd_voice_call_info_t));
    ...
    if (list_size(&jrd_voice_event_list) > 0) {
        list_node_link = list_peek_front(&jrd_voice_event_list);
    }

    while (list_node_link != NULL) {
        this_msg_node = (jrd_voice_event_list_node_t *)list_node_link;
        ...
        list_node_link =  (list_link_type *) list_peek_next(&jrd_voice_event_list, list_node_link);
...
*/
/*=========================================================================*/
unsigned char jrd_voice_al_get_call_id(void)
{
...
*/
/*=========================================================================*/
int jrd_voice_al_get_call_state(void)
{
...
*/
/*=========================================================================*/
int jrd_voice_al_get_call_number(char *number)
{
...
*/
/*=========================================================================*/
unsigned char jrd_voice_al_get_third_call_id(void)
{
...
*/
/*=========================================================================*/
int jrd_voice_al_get_third_call_state(void)
{
...
*/
/*=========================================================================*/
int jrd_voice_al_get_third_call_number(char *number)
{
...
*/
/*=========================================================================*/
int jrd_voice_al_clear_third_call_state(void)
{
...
*/
/*=========================================================================*/
int jrd_voice_al_manage_calls(void)
{
    int rc = JRD_NO_ERR;
    sups_type_enum_v02 sups_type = SUPS_TYPE_CCBS_ACTIVATION_V02; // 0x07

    rc = jrd_qmi_voice_manage_calls(g_call_info.call_id, sups_type);
    if (rc != QMI_NO_ERR) {
...
*/
/*=========================================================================*/
int jrd_voice_al_call_answer(void)
{
    int rc = JRD_NO_ERR;
    ...
    {
        rc = jrd_qmi_voice_call_answer(g_third_call_info.call_id);
        if (rc != QMI_NO_ERR) {
    ...
    {
        rc = jrd_qmi_voice_call_answer(g_call_info.call_id);
        if (rc != QMI_NO_ERR) {
            JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR,"Error, answer call failed.\n");
            return JRD_FAIL;
        }
...
*/
/*=========================================================================*/
int jrd_voice_al_call_answer_busy(void)
{
    int rc = JRD_NO_ERR;

    rc = jrd_qmi_voice_call_answer_busy(g_call_info.call_id);
    if (rc != QMI_NO_ERR) {
        JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR,"Error, answer call failed.\n");
        return JRD_FAIL;
    }
...
/*=========================================================================*/
#if 1
void *jrd_voice_al_digit_dtmf_thread (void *data)
{
    int rc = 0;
    uint32 req_signal = 0;
    char digit_dtmf_num = 0;
    jrd_voice_dtmf_queue_t *p_dtmf_queue = &jrd_dtmf_queue;

    while (1)
    {
        JRD_LOCK (&jrd_key_signal);
        
        /* Dtmf queue is empty, waiting for new key_signal. */
        if (p_dtmf_queue->head == p_dtmf_queue->tail)
        {
            pthread_cond_wait (&jrd_key_signal.cond, &jrd_key_signal.mutex);
        }
        ...
        while (p_dtmf_queue->head < p_dtmf_queue->tail)
        {
            ...
            rc = jrd_qmi_voice_start_dtmf (g_call_info.call_id, digit_dtmf_num);
            if (rc != QMI_NO_ERR)
            {
                JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "Error, dtmf number send failed.\n");
                return NULL;
            } else {
                //pthread_cond_wait (&jrd_net_signal.cond, &jrd_net_signal.mutex);
                JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR, "--->>[START]<<--- cond_wait recvived signal.\n");
            }
            
            /* Stop dtmf after recv ack success. */
            rc = jrd_qmi_voice_stop_dtmf (g_call_info.call_id);
            if (rc != QMI_NO_ERR)
            {
...
*/
/*=========================================================================*/
int jrd_voice_al_call_dial(char *call_number, int dial_type)
{
    int rc = JRD_NO_ERR;
    ...
    rc = jrd_qmi_voice_call_dial(&g_call_info.call_id, call_number, JRD_STRLEN(call_number), dial_type);
    if (rc != QMI_NO_ERR) {
...
*/
/*=========================================================================*/
int jrd_voice_al_call_end(void)
{
    int rc = JRD_NO_ERR;
    ...
    {
        rc = jrd_qmi_voice_call_end(g_third_call_info.call_id);
        if (rc != QMI_NO_ERR) {
            JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR,"Error, end call failed.\n");
            return JRD_FAIL;
        }
    } else {
        rc = jrd_qmi_voice_call_end(g_call_info.call_id);
        if (rc != QMI_NO_ERR) {
            JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR,"Error, end call failed.\n");
            return JRD_FAIL;
        }
...
*/
/*=========================================================================*/
int jrd_voice_al_event_register(void *callback, void *cb_param)
{
    jrd_voice_event_list_node_t *new_event_node = NULL;
    ...
    JRD_MALLOC(sizeof(jrd_voice_event_list_node_t), new_event_node);
    if (!new_event_node) {
    ...
    list_push_back(&jrd_voice_event_list, (list_link_type *)new_event_node);
    return JRD_NO_ERR;
}
...
*/
/*=========================================================================*/
int jrd_voice_al_event_unregister(void *callback)
{
    jrd_voice_event_list_node_t *del_event_node = NULL;
    ...
    del_event_node = (jrd_voice_event_list_node_t *)list_peek_front(&jrd_voice_event_list);
    while (del_event_node != NULL) {
        if (del_event_node->event_element.callback == callback) {
            list_pop_item(&jrd_voice_event_list, (list_link_type *)del_event_node);
            JRD_FREE(del_event_node);
            del_event_node = NULL;
        } else {
            del_event_node = (jrd_voice_event_list_node_t *)list_peek_next(&jrd_voice_event_list, del_event_node->link);
        }
...
*/
/*=========================================================================*/
int jrd_voice_al_init(void)
{
    int rc = JRD_NO_ERR;

    /* Register voice indication msg. */
    rc = jrd_qmi_voice_msg_register(jrd_voice_al_msg_cb);
    if (rc != QMI_NO_ERR) {
        JRD_OEM_LOG_INFO(JRD_OEM_LOG_ERROR,"Error, register qmi voice msg.\n");
        return JRD_FAIL;
    }

    jrd_qmi_voice_indication_register();
    g_call_info.call_id = 255;

#if 1
    JRD_OS_SIGNAL_INIT (&jrd_key_signal);
    JRD_OS_SIGNAL_INIT (&jrd_net_signal);
    if (pthread_create(&jrd_dtmf_thrd, NULL, jrd_voice_al_digit_dtmf_thread, NULL) < 0)
    {
...
*/
/*=========================================================================*/
int jrd_voice_al_exit(void)
{
    /* Unregister. */
    jrd_qmi_voice_msg_unregister(jrd_voice_al_msg_cb);
    return JRD_NO_ERR;
}
