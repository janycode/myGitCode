...
macro ExpandProcCN(szMyName,wordinfo,szLine,szLine1,nVer,ln,sel)
{
    ...
    {   
        if(wordinfo.ichLim > 70)
        {
            Msg("ÓÒ±ß¿Õ¼äÌ«Ð¡,ÇëÓÃÐÂµÄÐÐ")
            stop 
        }        szCurLine = GetBufLine(hbuf, sel.lnFirst);
        szLeft = strmid(szCurLine,0,wordinfo.ichLim)
        lineLen = strlen(szCurLine)
        kk = 0
        /*×¢ÊÍÖ»ÄÜÔÚÐÐÎ²£¬±ÜÃâ×¢ÊÍµôÓÐÓÃ´úÂë*/
        while(wordinfo.ichLim + kk < lineLen)
        {
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    else if (szCmd == "func" || szCmd == "fu")
    {
        ...
        {
            szNextLine = GetBufLine(hbuf,ln)
            /*¶ÔÓÚ2.1°æµÄsiÈç¹ûÊÇ·Ç·¨symbol¾Í»áÖÐ¶ÏÖ´ÐÐ£¬¹Ê¸ÃÎªÒÔºóÒ»ÐÐ
              ÊÇ·ñÓÐ¡®£¨¡¯À´ÅÐ¶ÏÊÇ·ñÊÇÐÂº¯Êý*/
            if( (strstr(szNextLine,"(") != 0xffffffff) || (nVer != 2))
            {
                /*ÊÇÒÑ¾­´æÔÚµÄº¯Êý*/
                symbol = GetCurSymbol()
    ...
    {   
        ...
        DelBufLine(hbuf, ln)
        szQuestion = AddPromblemNo()
        InsBufLine(hbuf, ln, "@szLine1@/* ÎÊ Ìâ µ¥: @szQuestion@     ÐÞ¸ÄÈË:@szMyName@,   Ê±¼ä:@sz@/@sz1@/@sz3@ ");
        szContent = Ask("ÐÞ¸ÄÔ­Òò")
        szLeft = cat(szLine1,"   ÐÞ¸ÄÔ­Òò: ");
        if(strlen(szLeft) > 70)
        {
            Msg("ÓÒ±ß¿Õ¼äÌ«Ð¡,ÇëÓÃÐÂµÄÐÐ")
            stop 
    ...
    else if (szCmd == "ab")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Added by @szMyName@, @sz@/@sz1@/@sz3@   ÎÊÌâµ¥ºÅ:@szQuestion@ */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Added by @szMyName@, @sz@/@sz1@/@sz3@ */");        
    ...
    else if (szCmd == "db")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Deleted by @szMyName@, @sz@/@sz1@/@sz3@   ÎÊÌâµ¥ºÅ:@szQuestion@ */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Deleted by @szMyName@, @sz@/@sz1@/@sz3@ */");
    ...
    else if (szCmd == "mb")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Modified by @szMyName@, @sz@/@sz1@/@sz3@   ÎÊÌâµ¥ºÅ:@szQuestion@ */");
        }
        else
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Modified by @szMyName@, @sz@/@sz1@/@sz3@ */");
...
//        ret =strstr(szLine,"return")
    ...
    //Ö»ÒªÇ°ÃæµÄreturnºóÓÐÒ»¸ö}ÁËËµÃ÷º¯ÊýµÄ½áÎ²Ã»ÓÐ·µ»Ø£¬ÐèÒªÔÙ¼ÓÒ»¸ö³ö¿Ú´òÓ¡
    if(fIsNeedPrt == 1)
    {
        InsBufLine(hbuf, ln,  "    DebugTrace(\"\\r\\n |@symbolname@() exit---: @nExitCount@ \");")        
        InsBufLine(hbuf, ln,  "")        
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "                  °æÈ¨ËùÓÐ (C), 2001-2011, ÉñÖÝÊýÂëÍøÂçÓÐÏÞ¹«Ë¾")
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
    InsBufLine(hbuf, ln + 5,  "  ÎÄ ¼þ Ãû   : @sz@")
    InsBufLine(hbuf, ln + 6,  "  °æ ±¾ ºÅ   : ³õ¸å")
    InsBufLine(hbuf, ln + 7,  "  ×÷    Õß   : @szName@")
    SysTime = GetSysTime(1)
    szTime = SysTime.Date
    InsBufLine(hbuf, ln + 8,  "  Éú³ÉÈÕÆÚ   : @szTime@")
    InsBufLine(hbuf, ln + 9,  "  ×î½üÐÞ¸Ä   :")
    iLen = strlen (szContent)
    nlnDesc = ln
    szTmp = "  ¹¦ÄÜÃèÊö   : "
    InsBufLine(hbuf, ln + 10, "  ¹¦ÄÜÃèÊö   : @szContent@")
    InsBufLine(hbuf, ln + 11, "  º¯ÊýÁÐ±í   :")
    
    //²åÈëº¯ÊýÁÐ±í
    ln = InsertFileList(hbuf,hnewbuf,ln + 12) - 12
    closebuf(hnewbuf)
    InsBufLine(hbuf, ln + 12, "  ÐÞ¸ÄÀúÊ·   :")
    InsBufLine(hbuf, ln + 13, "  1.ÈÕ    ÆÚ   : @szTime@")
    ...
    InsBufLine(hbuf, ln + 15, "    ÐÞ¸ÄÄÚÈÝ   : ´´½¨ÎÄ¼þ")    
    InsBufLine(hbuf, ln + 16, "")
    InsBufLine(hbuf, ln + 17, "******************************************************************************/")
    InsBufLine(hbuf, ln + 18, "")
    InsBufLine(hbuf, ln + 19, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 20, " * °üº¬Í·ÎÄ¼þ                                   *")
    InsBufLine(hbuf, ln + 21, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 22, "")
    InsBufLine(hbuf, ln + 23, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 24, " * Íâ²¿±äÁ¿ËµÃ÷                                 *")
    InsBufLine(hbuf, ln + 25, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 26, "")
    InsBufLine(hbuf, ln + 27, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 28, " * Íâ²¿º¯ÊýÔ­ÐÍËµÃ÷                             *")
    InsBufLine(hbuf, ln + 29, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 30, "")
    InsBufLine(hbuf, ln + 31, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 32, " * ÄÚ²¿º¯ÊýÔ­ÐÍËµÃ÷                             *")
    InsBufLine(hbuf, ln + 33, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 34, "")
    InsBufLine(hbuf, ln + 35, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 36, " * È«¾Ö±äÁ¿                                     *")
    InsBufLine(hbuf, ln + 37, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 38, "")
    InsBufLine(hbuf, ln + 39, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 40, " * Ä£¿é¼¶±äÁ¿                                   *")
    InsBufLine(hbuf, ln + 41, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 42, "")
    InsBufLine(hbuf, ln + 43, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 44, " * ³£Á¿¶¨Òå                                     *")
    InsBufLine(hbuf, ln + 45, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 46, "")
    InsBufLine(hbuf, ln + 47, "/*----------------------------------------------*")
    InsBufLine(hbuf, ln + 48, " * ºê¶¨Òå                                       *")
    InsBufLine(hbuf, ln + 49, " *----------------------------------------------*/")
    InsBufLine(hbuf, ln + 50, "")
    ...
    //Èç¹ûÃ»ÓÐÊäÈë¹¦ÄÜÃèÊöµÄ»°ÌáÊ¾ÊäÈë
    szContent = Ask("ÇëÊäÈëÎÄ¼þ¹¦ÄÜÃèÊöµÄÄÚÈÝ")
    SetBufIns(hbuf,nlnDesc + 14,0)
    DelBufLine(hbuf,nlnDesc +10)
    
    //×Ô¶¯ÅÅÁÐÏÔÊ¾¹¦ÄÜÃèÊö
    CommentContent(hbuf,nlnDesc+10,"  ¹¦ÄÜÃèÊö   : ",szContent,0)
}
...
macro CommentContent1 (hbuf,ln,szPreStr,szContent,isEnd)
{
    ...
    //Èç¹ûÊäÈë´°¿ÚÖÐµÄÄÚÈÝÊÇ¼ôÌù°åÖÐµÄÄÚÈÝËµÃ÷ÊÇ¼ôÌù¹ýÀ´µÄ
...
macro GetWordFromString(hbuf,szLine,nBeg,nEnd,chBeg,chSeparator,chEnd)
{
    ...
    //ÓÃÓÚ¼ì²âchBegºÍchEndµÄÅä¶ÔÇé¿ö
    iCount = 0
    
    nEndWord = 0
    //ÒÔ·Ö¸ô·ûÎª±ê¼Ç½øÐÐËÑË÷
    while(nIdx < nEnd)
    {
...
macro FuncHeadCommentCN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    if( strlen(szFunc)>0 )
    {
        InsBufLine(hbuf, ln+1, " º¯ Êý Ãû  : @szFunc@")
    }
    else
    {
        InsBufLine(hbuf, ln+1, " º¯ Êý Ãû  : #")
    }
    oldln = ln
    InsBufLine(hbuf, ln+2, " ¹¦ÄÜÃèÊö  : ")
    szIns = " ÊäÈë²ÎÊý  : "
    if(newFunc != 1)
    {
        //¶ÔÓÚÒÑ¾­´æÔÚµÄº¯Êý²åÈëº¯Êý²ÎÊý
        i = 0
        while ( i < lnMax) 
        {
    ...
    {       
            ln = ln + 1
            InsBufLine(hbuf, ln+2, " ÊäÈë²ÎÊý  : ÎÞ")
    }
    InsBufLine(hbuf, ln+3, " Êä³ö²ÎÊý  : ÎÞ")
    InsBufLine(hbuf, ln+4, " ·µ »Ø Öµ  : @szRet@")
    InsBufLine(hbuf, ln+5, " µ÷ÓÃº¯Êý  : ")
    InsBufLine(hbuf, ln+6, " ±»µ÷º¯Êý  : ")
    InsbufLIne(hbuf, ln+7, " ");
    InsBufLine(hbuf, ln+8, " ÐÞ¸ÄÀúÊ·      :")
    SysTime = GetSysTime(1);
    szTime = SysTime.Date

    InsBufLine(hbuf, ln+9, "  1.ÈÕ    ÆÚ   : @szTime@")

    if( strlen(szMyName)>0 )
    {
       InsBufLine(hbuf, ln+10, "    ×÷    Õß   : @szMyName@")
    }
    else
    {
       InsBufLine(hbuf, ln+10, "    ×÷    Õß   : #")
    }
    InsBufLine(hbuf, ln+11, "    ÐÞ¸ÄÄÚÈÝ   : ÐÂÉú³Éº¯Êý")    
    ...
    sel.lnFirst = ln + 14
    sel.lnLast = ln + 14        
    szContent = Ask("ÇëÊäÈëº¯Êý¹¦ÄÜÃèÊöµÄÄÚÈÝ")
    setWndSel(hwnd,sel)
    DelBufLine(hbuf,oldln + 2)

    //ÏÔÊ¾ÊäÈëµÄ¹¦ÄÜÃèÊöÄÚÈÝ
    newln = CommentContent(hbuf,oldln+2," ¹¦ÄÜÃèÊö  : ",szContent,0) - 2
    ln = ln + newln - oldln
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        isFirstParam = 1
            
        //ÌáÊ¾ÊäÈëÐÂº¯ÊýµÄ·µ»ØÖµ
        szRet = Ask("ÇëÊäÈë·µ»ØÖµÀàÐÍ")
        if(strlen(szRet) > 0)
        {
            PutBufLine(hbuf, ln+4, " ·µ »Ø Öµ  : @szRet@")            
...
macro InsertHistory(hbuf,ln,language)
{
    ...
    while(ln-i>0)
    {
        ...
        iBeg1 = strstr(szCurLine,"ÐÞ¸ÄÀúÊ·")
...
macro UpdateFunctionList()
{
    ...
    while(ln < iTotalLn)
    {
        ...
        //ÒÔÎÄ¼þÍ·ËµÃ÷ÖÐÇ°ÓÐ´óÓÚ10¸ö¿Õ¸ñµÄÎªº¯ÊýÁÐ±í¼ÇÂ¼
        if(j > 10)
        {
            DelBufLine(hbuf, ln)   
        }
...
macro  InsertHistoryContentCN(hbuf,ln,iHostoryCount)
{
    ...
    else
    {
       ...
       szContent = Ask("ÇëÊäÈëÐÞ¸ÄµÄÄÚÈÝ")
       CommentContent(hbuf,ln + 3,"    ÐÞ¸ÄÄÚÈÝ   : ",szContent,0)
}
...
macro ExpandBraceLarge()
{
    ...
    {
        //¶ÔÓÚÓÐ¿éÑ¡ÔñµÄÇé¿ö»¹µÃ¿¼ÂÇ½«¿éÑ¡ÔñÇø·Ö¿ªÁË
        
        //¼ì²éÑ¡ÔñÇøÄÚÊÇ·ñ´óÀ¨ºÅÅä¶Ô£¬Èç¹ûÏÓÌ«ÂýÔò×¢ÊÍµôÏÂÃæµÄÅÐ¶Ï
        ...
        if(sel.lnFirst != sel.lnLast)
        {
            ...
            {
                //Èç¹ûÑ¡ÔñÇø³¤¶È´óÓÚ¸ÄÐÐµÄ³¤¶È£¬×î´óÈ¡¸ÃÐÐµÄ³¤¶È
        ...
        if(strlen(szRight) != 0)
        {
            //×îºó²åÈë×îºóÒ»ÐÐÃ»ÓÐ±»Ñ¡ÔñµÄÄÚÈÝ
        ...
        {
            //Èç¹ûÑ¡ÖÐÇøÇ°µÄÄÚÈÝ²»ÊÇ¿Õ¸ñ£¬ÔòÒª±£Áô¸Ã²¿·ÖÄÚÈÝ
        ...
        {
            //Èç¹ûÑ¡ÖÐÇøÇ°Ã»ÓÐÄÚÈÝÖ±½ÓÉ¾³ý¸ÃÐÐ
            DelBufLine(hbuf,ln)
            InsBufLine(hbuf, ln, "@szLeft@{")
        }
        if(strlen(szMid) > 0)
        {
            //²åÈëµÚÒ»ÐÐÑ¡ÔñÇøµÄÄÚÈÝ
...
macro DelCompoundStatement()
{
    ...
    while(1)
    {
        ...
        if(ret != 0xffffffff)
        {
            ...
            //Ê¹ÓÃSiµÄ´óÀ¨ºÅÅä¶Ô·½·¨£¬µ«V2.1Ê±ÔÚ×¢ÊÍÇ¶Ì×Ê±¿ÉÄÜÓÐÎó
/*            SetWndSel(hwnd,sel)
            Block_Down
            selNew = GetWndSel(hwnd)
            if(selNew != sel)
    ...
    while ( lnCurrent <= lnLast )
    {
        ...
        if( isCommentEnd == 1 )
        {
            ...
            if(( szLine[ich]==ch_comment ) && (szLine[ich+1]==ch_comment))
            {
                ...
                {
                    /*Èç¹û²»ÊÇÁ¬ÐøµÄ×¢ÊÍÔòÊÇÐÂ×¢ÊÍµÄ¿ªÊ¼*/
                    szLine[ich] = "/"
                    szLine[ich+1] = "*"
                }
                if ( lnCurrent == lnLast )
                {
                    /*Èç¹ûÊÇ×îºóÒ»ÐÐÔòÔÚÐÐÎ²Ìí¼Ó½áÊø×¢ÊÍ·û*/
                    szLine = cat(szLine,"  */")
                    isCommentContinue = 0
                }
                /*¸üÐÂ¸ÃÐÐ*/
